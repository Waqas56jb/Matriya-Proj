...
export function buildMatriyaCatalogAppendix(filenames) {
  ...
  return (
    '\n\n[System — indexed document names in Matriya. When the user names a file, map to these paths. User-visible text built from file_search must be verbatim quotes from snippets (no paraphrase of document wording). Every factual claim must be supported only by file_search; do not use general knowledge. If nothing answers the question, say so clearly in Hebrew.]\n' +
    unique.map((n) => `· ${n}`).join('\n') +
    '\n'
  );
}

/** Search / RAG path: evidence for kernel and agents (`forContextOnly: true`). Quotes must stay verbatim. */
const INSTRUCTIONS_CONTEXT =
  'You retrieve evidence from file_search for downstream agents. Use ONLY retrieved snippet text from the attached vector store. ' +
  'Copy excerpts VERBATIM (same characters, punctuation, numbers as in the snippet); do not paraphrase, summarize, or reword document text inside quoted blocks. ' +
  'Label each excerpt with its source filename as in search results. Short Hebrew headings outside quotes are OK when the source is Hebrew. ' +
  'Do not use general knowledge for factual content. If retrieval has nothing relevant, state briefly in Hebrew — do not invent.';

/** Ask Matriya + answered search (`forContextOnly: false`). Same verbatim rule as management GPT RAG. */
export const MATRIYA_FILE_SEARCH_INSTRUCTIONS_ANSWER =
  ...
  'VERBATIM QUOTES (critical): The substantive answer must be DIRECT copies from retrieved snippet text — same words, punctuation, and numbers; use « » or quotation marks. Do not paraphrase, summarize, translate, or "clean up" text inside quotes. ' +
  'You may add only minimal Hebrew framing outside quotes (e.g. "לפי [שם הקובץ]:"). Quoted document text stays in its original language; Hebrew (עברית) for labels and explanations outside quotes only. Do not use Arabic. ' +
  'For several documents, separate quotes with the filename before each; do not merge into one paraphrased sentence. ' +
  'Do NOT use general knowledge, training data, or the web for facts. If documents do not contain the answer, say clearly in Hebrew — never substitute from memory. ' +
  'When the user names a file, prioritize snippets from that file. For cross-document questions, quote only what appears in retrieval, each with its source filename.';
