...
export function buildMatriyaCatalogAppendix(filenames) {
  ...
  return (
    '\n\n[System — indexed document names in Matriya. Answer policy: מותר לקצר ולארגן ציטוטים מ-file_search למשפטים ברורים; אסור להוסיף מידע, להשלים פערים או להסיק מעבר לציטוטים — טרנספורמציה של הציטוטים בלבד. No general knowledge. If nothing answers, say so in Hebrew.]\n' +
    unique.map((n) => `· ${n}`).join('\n') +
    '\n'
  );
}
...
const INSTRUCTIONS_CONTEXT =
  ...
  'Label excerpts with source filenames. No general knowledge for facts. If nothing relevant, state briefly in Hebrew — do not invent.';
...
export const MATRIYA_FILE_SEARCH_INSTRUCTIONS_ANSWER =
  ...
  'English: You may shorten and reorder quoted retrieval text into clear sentences; every factual claim must remain anchored in what those excerpts state — no new facts, no gap-filling, no inference beyond the quotes. ' +
  'Cite source filenames. Respond in Hebrew (עברית) unless the user explicitly asks otherwise. Do not use Arabic. ' +
  'Do NOT use general knowledge, training data, or the web for facts. If documents do not contain the answer, say clearly in Hebrew. ' +
